...
- **[aws-bedrock-iac-reference](https://github.com/Ariel-J-Lee/aws-bedrock-iac-reference)** — Bedrock-anchored AWS reference architecture as Terraform IaC, surfacing security, cost, observability, and cloud-hygiene judgment. Tier-4 dry-run evidence is live on `main`: `terraform validate` + `plan` + `tfsec` + `checkov` outputs under `plans/canonical/`.
